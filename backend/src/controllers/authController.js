...
exports.register = async (req, res, next) => {
  try {
    ...
    sendEmail({ email: user.email, subject: 'Welcome to Rajvati Fashion', html: getWelcomeEmail(user.fullname) }).catch(err => console.log(err));
...
exports.login = async (req, res, next) => {
  try {
    ...
    sendEmail({ email: user.email, subject: 'Secure Login - Rajvati Fashion', html: getLoginNotificationEmail(user.fullname) }).catch(e => console.log(e));

    let permissions = [];
    if (user.role === 'super_admin') {
      permissions = ['*'];
    } else if (user.role === 'admin' && user.adminRole) {
    ...
    res.status(200).json({
      ...
      data: {
        user: { _id: user._id, fullname: user.fullname, email: user.email, role: user.role, permissions },
        token, expiresAt: getExpiresAt()
      }
    });
  } catch (error) { next(error); }
